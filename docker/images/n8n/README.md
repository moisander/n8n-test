...
The Dockerfile uses a multi-stage build that can either build n8n from source
or use a pre-compiled `compiled/` directory if one is already present.

### Standalone Docker Build

If your build server (e.g. Railway, Render, Coolify) runs `docker build`
...
from source inside Docker automatically.

### Local Development Build

For the fastest local workflow that handles both compilation and image creation:

```bash
pnpm build:docker
```

This runs `pnpm build:deploy` first (creating `compiled/`), then builds the
Docker image. The builder stage detects the existing `compiled/` directory and
skips the in-Docker build.

### CI / Pre-compiled Build

If your CI pipeline pre-compiles n8n separately, build the application first:

```bash
pnpm run build:deploy
```

Then run the Docker build. The builder stage will pass through the pre-compiled
